...
contract('TroveManager', async accounts => {
  ...
  */
  ...
  /* ABDK64, no error correction:
    ...
    PUSDDebt left in Default Pool is: 108
    ...
    PUSDDebt left in Default Pool is: 653
  */
  ...
  /* ABDK64, no error correction:
    ...
    PUSDDebt left in Default Pool is: 75
  ...
  */
  ...
  /* ABDK64, no error correction:
    ...
    PUSDDebt left in Default Pool is: 180
  ...
  */
  ...
  it("11 accounts. 10 liquidations. Check (ActivePool - DefaultPool) differences", async () => {
    ...
    let totalPUSDDebtPoolDifference = web3.utils.toBN(0)

    for (account of accounts.slice(1, 11)) {
      const activePoolETH = await activePool.getCollateral(weth.address)
      const activePoolPUSDDebt = await activePool.getPUSD()
      ...
      totalPUSDDebtPoolDifference.add(activePoolPUSDDebt.sub(defaultPoolPUSDDebt))
    }
    
    console.log(`Accumulated ETH difference between Default and Active Pools is: ${totalETHPoolDifference}`)
    console.log(`Accumulated PUSDDebt difference between Active and Default Pools is: ${totalPUSDDebtPoolDifference}`)
  })
  
  /* ABDK64, no error correction
    Accumulated ETH difference between Default and Active Pools is: 0
    Accumulated PUSDDebt difference between Active and Default Pools is: 0
  ...
  */
  ...
  it("11 accounts. 10 liquidations. Check (DefaultPool - totalRewards) differences", async () => {
    ...
    const L_PUSDDebt = await troveManager.L_PUSDDebt()
    ...
    const totalPUSDRewards = (totalColl.mul(L_PUSDDebt)).div(_1e18_BN)

    const defaultPoolETH = await defaultPool.getCollateral(weth.address)
    const defaultPoolPUSDDebt = await defaultPool.getPUSDDebt()

    const ETHRewardDifference = defaultPoolETH.sub(totalETHRewards)
    const PUSDDebtRewardDifference = defaultPoolPUSDDebt.sub(totalPUSDRewards)

    console.log(`ETH difference between total pending rewards and DefaultPool: ${ETHRewardDifference} `)
    console.log(`PUSDDebt difference between total pending rewards and DefaultPool: ${PUSDDebtRewardDifference} `)
  })

  /* ABDK64, no error correction:
    ETH difference between total pending rewards and DefaultPool: 700
    PUSDDebt difference between total pending rewards and DefaultPool: 800

    ABDK64 WITH correction:
    ETH difference between total pending rewards and DefaultPool: 300
    PUSDDebt difference between total pending rewards and DefaultPool: 400
    
    DeciMath, no error correction:
    ETH difference between total pending rewards and DefaultPool: -100
    PUSDDebt difference between total pending rewards and DefaultPool: -200

    Pure division with correction: 
    ETH difference between total pending rewards and DefaultPool: 0
    PUSDDebt difference between total pending rewards and DefaultPool: 0
  */
  ...
  /* ABDK64, no error correction:
    ...
    PUSDDebt difference between total pending rewards and DefaultPool: 55000
    
    ABDK64 WITH correction:
    ETH difference between total pending rewards and DefaultPool: 31000
    PUSDDebt difference between total pending rewards and DefaultPool: 31000

    DeciMath, no error correction:
    ETH difference between total pending rewards and DefaultPool: 2000
    PUSDDebt difference between total pending rewards and DefaultPool: -2000
 ...
 it("11 accounts with random ETH and proportional PUSD (180:1). 10 liquidations. Check (DefaultPool - totalRewards) differences", async () => {
  await borrowerOperations.openTrove(0, 0,  accounts[999], { from: accounts[999], value: dec(100, 'ether') })

  await th.openTrove_allAccounts_randomETH_ProportionalPUSD(1, 2, accounts.slice(0, 11), contracts, 180)
  ...
  /* ABDK64, no error correction:
    ...
    PUSDDebt difference between total pending rewards and DefaultPool: 8000

    ABDK64 WITH correction:
    ETH difference between total pending rewards and DefaultPool: 300
    PUSDDebt difference between total pending rewards and DefaultPool: 300
    ...
    Pure division with correction:
    ETH difference between total pending rewards and DefaultPool: 100
    PUSDDebt difference between total pending rewards and DefaultPool: 100
  */

  it("101 accounts with random ETH and proportional PUSD (180:1). 100 liquidations. Check 1) (DefaultPool - totalDistributionRewards) difference, and 2) ", async () => {
    await borrowerOperations.openTrove(0, 0,  accounts[999], { from: accounts[999], value: dec(1000, 'ether') })

    await th.openTrove_allAccounts_randomETH_ProportionalPUSD(1, 2, accounts.slice(0, 101), contracts, 180)
  ...
  /* ABDK64, no error correction:
    ...
    PUSDDebt difference between total pending rewards and DefaultPool: 61000

    ABDK64 WITH correction:
    ETH difference between total pending rewards and DefaultPool: 31300
    PUSDDebt difference between total pending rewards and DefaultPool: 30000
    ...
    PUSDDebt difference between total pending rewards and DefaultPool: 1000
  */
  ...
  it("11 accounts. 10 liquidations, partial offsets. Check (DefaultPool - totalRewards) differences", async () => {
   // Acct 99 opens trove with 100 PUSD
    await borrowerOperations.openTrove(0, 0,  accounts[99], { from: accounts[99], value: dec(100, 'ether') })
    await borrowerOperations.withdrawPUSD(0, dec(100, 18), accounts[99], {from: accounts[99]})
    ...
    // On loop: Account[99] adds 10 PUSD to pool -> a trove gets liquidated and partially offset against SP, emptying the SP
    for (account of accounts.slice(1, 11)) {
    ...
    const totalPUSDRewards_Distribution = (totalColl.mul(L_PUSDDebt)).div(_1e18_BN)

    const defaultPoolETH = await defaultPool.getCollateral(weth.address)
    const defaultPoolPUSDDebt = await defaultPool.getPUSDDebt()

    const ETHRewardDifference = defaultPoolETH.sub(totalETHRewards_Distribution)
    const PUSDDebtRewardDifference = defaultPoolPUSDDebt.sub(totalPUSDRewards_Distribution)

    console.log(`ETH difference between total pending distribution rewards and DefaultPool: ${ETHRewardDifference} `)
    console.log(`PUSDDebt difference between total pending distribution rewards and DefaultPool: ${PUSDDebtRewardDifference} `)
  })

  /* ABDK64, no error correction
    ETH difference between total pending distribution rewards and DefaultPool: 550
    PUSDDebt difference between total pending distribution rewards and DefaultPool: 600
    
    DeciMath, no error correction:
    ETH difference between total pending distribution rewards and DefaultPool: 150
    PUSDDebt difference between total pending distribution rewards and DefaultPool: -200
    
    Pure division with error correction:
    ETH difference between total pending distribution rewards and DefaultPool: 50
    PUSDDebt difference between total pending distribution rewards and DefaultPool: 0
  */

  it("101 accounts. 100 liquidations, partial offsets. Check (DefaultPool - totalRewards) differences", async () => {
    // Acct 99 opens trove with 100 PUSD
     await borrowerOperations.openTrove(0, 0,  accounts[999], { from: accounts[999], value: dec(100, 'ether') })
     await borrowerOperations.withdrawPUSD(0, dec(100, 18), accounts[999], {from: accounts[999]})
  ...
  /* ABDK64, no error correction
    ...
    PUSDDebt difference between total pending distribution rewards and DefaultPool: 8900
  ...
  */
  ...
  it("11 accounts. 10 Borrowers add to SP. 1 liquidation, 10 Borrowers withdraw all their SP funds", async () => {
    ...
    /* Sometimes, the error causes the last PUSD withdrawal from SP to underflow and fail.
    ...
    const SP_PUSD = await stabilityPool.getTotalPUSDDeposits()

    const SP_PUSD_Insufficiency = web3.utils.toBN(whaleSPDeposit).sub(SP_PUSD)

     // check Stability Pool
    console.log(`Surplus ETH left in in Stability Pool is ${SP_ETH}`)
    console.log(`PUSD insufficiency in Stability Pool is ${SP_PUSD_Insufficiency}`)
   })

   /* ABDK64, no error correction
      Sometimes subtraction overflows on last withdrawal from SP - error leaves insufficient PUSD in Pool.
      ...
      PUSD insufficiency in Stability Pool is 530
      
      DeciMath, no error correction:
      Surplus ETH left in in Stability Pool is 0
      PUSD insufficiency in Stability Pool is 0
   ...
   /* ABDK64, no error correction
    ...
    PUSD insufficiency in Stability Pool is 5300
   ...
   */
   ...
   it("11 accounts. 10 Borrowers add to SP, random PUSD amounts. 1 liquidation, 10 Borrowers withdraw all their SP funds", async () => {
    // Acct 99 opens trove with 100 PUSD
     await borrowerOperations.openTrove(0, 0,  accounts[999], { from: accounts[999], value: dec(100, 'ether') })
     await borrowerOperations.withdrawPUSD(0, dec(100, 18), accounts[999], {from: accounts[999]})
   ...
   /* ABDK64, no error correction
      ...
      PUSD insufficiency in Stability Pool is 442
      ...
      PUSD insufficiency in Stability Pool is 67

      Pure division with error correction:
      Surplus ETH left in in Stability Pool is 446
      PUSD insufficiency in Stability Pool is 507
    */

   it("101 accounts. 100 Borrowers add to SP, random PUSD amounts. 1 liquidation, 100 Borrowers withdraw all their SP funds", async () => {
    // Acct 99 opens trove with 100 PUSD
     await borrowerOperations.openTrove(0, 0,  accounts[999], { from: accounts[999], value: dec(100, 'ether') })
     await borrowerOperations.withdrawPUSD(0, dec(100, 18), accounts[999], {from: accounts[999]})
   ...
   /* ABDK64, no error correction
    ...
    PUSD insufficiency in Stability Pool is 1112

    DeciMath, no error correction:
    Surplus ETH left in in Stability Pool is 1373
    PUSD insufficiency in Stability Pool is -13

    Pure division with no error correction:
    Surplus ETH left in in Stability Pool is 4087
    PUSD insufficiency in Stability Pool is 1960

    Pure division with error correction:
    Surplus ETH left in in Stability Pool is 3072
    PUSD insufficiency in Stability Pool is 452
  */ 

 it("501 accounts. 500 Borrowers add to SP, random PUSD amounts. 1 liquidation, 500 Borrowers withdraw all their SP funds", async () => {
  // Acct 99 opens trove with 100 PUSD
   await borrowerOperations.openTrove(0, 0, accounts[999], { from: accounts[999], value: dec(100, 'ether') })
   await borrowerOperations.withdrawPUSD(0, dec(100, 18), accounts[999], {from: accounts[999]})
  ...
  /* ABDK64, no error correction:
    ...
    PUSD insufficiency in Stability Pool is -8445

    Pure division, no correction:
    Surplus ETH left in in Stability Pool is 18708
    PUSD insufficiency in Stability Pool is 25427

    Pure division with error correction:
    Surplus ETH left in in Stability Pool is 1573
    PUSD insufficiency in Stability Pool is 6037
  */ 
...
/* DeciMath:
  ...
  PUSDDebt left in Default Pool is: 308731912

  Pure division, correction:
  ETH left in Default Pool is: 1136050360
  PUSDDebt left in Default Pool is: 997601870

  Pure division, no correction:
  ETH left in Default Pool is: 810899932
  PUSDDebt left in Default Pool is: 535042995
*/
...
it("101 accounts with random large coll, magnitude ~1e8 ether. 1 liquidation. 500 accounts do a Trove operation (apply rewards)", async () => {
 ...
 /*
  ...
  PUSDDebt left in Default Pool is: 14696382412

  Pure division, correction:
  ETH left in Default Pool is: 9281255535
  PUSDDebt left in Default Pool is: 5854012464
  */
...
it("11 accounts with random ETH and proportional PUSD (180:1). 10 liquidations. Check (DefaultPool - totalRewards) differences", async () => {
  ...
  // Troves open with 100-200 million ether and proportional PUSD Debt
  await th.openTrove_allAccounts_randomETH_ProportionalPUSD(100000000, 200000000, accounts.slice(0, 11), contracts, 180)
...
/* 
  ...
  PUSDDebt difference between total pending rewards and DefaultPool: 12000000000

  Pure division with correction:
  ETH difference between total pending rewards and DefaultPool: 1000000000
  PUSDDebt difference between total pending rewards and DefaultPool: 1000000000
  */
  ...
  it("101 accounts with random ETH and proportional PUSD (180:1). 100 liquidations. Check 1) (DefaultPool - totalDistributionRewards) difference, and 2) ", async () => {
    ...
    // Troves open with 100-200 million ether and proportional PUSD Debt
    await th.openTrove_allAccounts_randomETH_ProportionalPUSD(100000000, 200000000, accounts.slice(0, 101), contracts, 180)
  ...
  /*
    ...
    PUSDDebt difference between total pending rewards and DefaultPool: 870000000000

    Pure division with correction:
    ETH difference between total pending rewards and DefaultPool: 10000000000
    PUSDDebt difference between total pending rewards and DefaultPool: 10000000000
  */
})
 ...
 Test to see if error is stable or grows. 
  ...
  1) Reward applications accumulate ETH and PUSDDebt error in DefaultPool

  2) Liquidations accumulate ETH and PUSDDebt error in DefaultPool
  ...
  5) StabilityPool Withdrawals can accumulate PUSDLoss in the StabilityPool (i.e. they distribute too much PUSD), and can block
