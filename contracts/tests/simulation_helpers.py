...
PUSD_GAS_COMPENSATION = 200.0
MIN_NET_DEBT = 1800.0
MAX_FEE = Wei(1e18)

"""# Ether price (exogenous)

Ether is the collateral for PUSD. The ether price $P_t^e$ follows
...
"""# PREON price
In the first month, the price of PREON follows

> $P_t^q = P_{t-1}^q (1+\zeta_t^q)(1+\sigma_t^q)$. 

Note that $\zeta_t^q \sim N(0,$ sd_PREON) represents PREON price shock and $\sigma_t^q$ the drift. Here, $\sigma_t^q =$ drift_PREON, so that the expected PREON price increases from price_PREON_initial to the following at the end of the first month:
> $E(P_{720}^q) = $price_PREON_initial$ \cdot (1+$ drift_PREON$)^{720}$

The PREON price from the second month on is endogenously determined.
"""

#PREON price & airdrop
price_PREON_initial = 0.4
...
"""**PREON Endogenous Price**

The staked PREON pool earning consists of the issuance fee revenue and redemption fee revenue
> $R_t^q = R_t^i + R_t^r.$
...
> $$r_t=\frac{PREON Market Cap}{Annualized Earning}=\frac{MC_t}{E_t}$$

> $MC_t=P_t^q \cdot$ PREON_total_supply

Therefore, the PREON price dynamics is determined
> $$P_t^q=discount \cdot \frac{r^{PE}}{PREON\_total\_supply}E_t$$

Interpretation: The denominator implies that with more PREON tokens issued, PREON price decreases. However, the depreciation effect can be counteracted by the growth of the earning.
...
$R_{t-1}^s$ is the return in the stability pool, which consists of liquidation gain and airdrop PREON gain.
...
> (collateral in ether, debt in PUSD, collateral ratio target, rational inattention, collateral ratio)
...
$P_{t}^{l}$ is the price of PUSD.

$N_t^o$ is rounded to an integer.

---

The amount of PUSD tokens generated by a new trove is
...
- $R_t^a=P_{t}^q\hat{Q}_t^q$ is the airdrop gain, $\hat{Q}_t^q=1000$ denotes the amount of PREON token airdropped to the stability pool providers
- $D_{t}^{s}$ is the total amount of PUSD deposited in the Stability Pool (see below)
...
def pending_liquidations(contracts, price_ether_current):
    ...
    stability_pool_balance = contracts.stabilityPool.getTotalPUSDDeposits()
    trove = last_trove
...
# F = 0.5 ** (1/8760)
# F = 0.99992087674
def quantity_PREON_airdrop(index):
    F = 0.99992087674
    if index <= 0:
        return 0
    return 32e6 * (F ** (index-1) - F ** index)

def liquidate_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_PUSD, price_PREON_current, data, index):
    if len(active_accounts) == 0:
        return [0, 0]

    stability_pool_previous = contracts.stabilityPool.getTotalPUSDDeposits() / 1e18
    ...
    while pending_liquidations(contracts, price_ether_current):
        try:
            ...
            for i in range(NUM_LIQUIDATIONS):
                ...
                ICR = contracts.troveManager.getCurrentICR(trove, Wei(price_ether_current * 1e18))
                print(f"ICR: {ICR}")
    stability_pool_current = contracts.stabilityPool.getTotalPUSDDeposits() / 1e18
    ...
    liquidation_gain = ether_liquidated * price_ether_current - debt_liquidated * price_PUSD
    airdrop_gain = price_PREON_current * quantity_PREON_airdrop(index)

    data['liquidation_gain'][index] = liquidation_gain
    data['airdrop_gain'][index] = airdrop_gain

    return_stability = calculate_stability_return(contracts, price_PUSD, data, index)

    return [ether_liquidated, return_stability]

def calculate_stability_return(contracts, price_PUSD, data, index):
    stability_pool_previous = contracts.stabilityPool.getTotalPUSDDeposits() / 1e18
    if index == 0:
        ...
        return_stability = (year/index) * \
            (sum(data['liquidation_gain'][0:index]) +
             ...
             ) / (price_PUSD * stability_pool_previous)
    else:
...
def close_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_PUSD, index):
    if len(active_accounts) == 0:
        return [0]
    ...
    if index <= 240:
        ...
        number_closetroves = max(0, n_steady * (1+shock_closetroves)) + beta*(1-price_PUSD)*n_troves
    ...
    for i in range(0, len(drops)):
        ...
        coll = amounts['coll']
        debt = amounts['debt']
        pending = get_pusd_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt)
        if pending == 0:
...
def transfer_from_to(contracts, from_account, to_account, amount):
    balance = contracts.pusdToken.balanceOf(from_account)
    transfer_amount = min(balance, amount)
    if transfer_amount == 0:
        return amount
    if from_account == to_account:
        return amount
    contracts.pusdToken.transfer(to_account, transfer_amount, { 'from': from_account })
    pending = amount - transfer_amount

    return pending

def get_pusd_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt):
    pusdBalance = contracts.pusdToken.balanceOf(account)
    if debt > pusdBalance:
        pending = debt - pusdBalance
        ...
        # try with active accounts, which are more likely to hold PUSD
        ...
        if pending > 0:
            print(f"\n ***Error: not enough PUSD to repay! {debt / 1e18} PUSD for {account}")
...
def adjust_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, index):
    ...
    for i, working_trove in enumerate(active_accounts):
        ...
        if p >= ratio:
            ...
            if check < -1:
                ...
                pending = get_pusd_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, repay_amount)
                if pending == 0:
                    contracts.borrowerOperations.repayPUSD(repay_amount, hints[0], hints[1], { 'from': account })
                ...
                if isNewTCRAboveCCR(contracts, 0, False, withdraw_amount_wei, True, floatToWei(price_ether_current)):
                    contracts.borrowerOperations.withdrawPUSD(MAX_FEE, withdraw_amount_wei, hints[0], hints[1], { 'from': account })
                    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
                    issuance_PUSD_adjust = issuance_PUSD_adjust + rate_issuance * withdraw_amount
    ...
    return [coll_added_float, issuance_PUSD_adjust]
...
def open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_ether, CR_ratio, rational_inattention, price_ether_current):
    ...
    debtChange = floatToWei(supply_trove) + PUSD_GAS_COMPENSATION
    pusd = get_pusd_amount_from_net_debt(contracts, floatToWei(supply_trove))
    if isNewTCRAboveCCR(contracts, coll, True, debtChange, True, floatToWei(price_ether_current)):
        contracts.borrowerOperations.openTrove(MAX_FEE, pusd, hints[0], hints[1],
...
def open_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_PUSD, index):
    ...
    elif price_PUSD <= 1 + rate_issuance:
        number_opentroves = max(0, n_steady * (1+shock_opentroves))
    else:
        number_opentroves = max(0, n_steady * (1+shock_opentroves)) + \
                        alpha * (price_PUSD - rate_issuance - 1) * n_troves
    ...
    return [coll_added, issuance_PUSD_open]
...
def stability_update(accounts, contracts, active_accounts, return_stability, index):
    supply = contracts.pusdToken.totalSupply() / 1e18
    stability_pool_previous = contracts.stabilityPool.getTotalPUSDDeposits() / 1e18
    ...
    if stability_pool > stability_pool_previous:
        ...
        while remaining > 0 and i < len(active_accounts):
          ...
          balance = contracts.pusdToken.balanceOf(account) / 1e18
        ...
        current_deposit = contracts.stabilityPool.getCompoundedPUSDDeposit(accounts[0])
...
---
With the supply and demand of PUSD tokens defined above, the price of PUSD at the current period is given by the following equilibrium condition:
> $$S_t = D_t^s + D_t^l = D_t^s + D_{t-1}^l (1+\zeta_t^l)(1+\sigma_t^l) (\frac{P_t^l}{P_{t-1}^l})^\delta$$

where $S$ is the total supply of PUSD.

Solving this equation gives that:
> $$P_t^l = P_{t-1}^l (\frac{S_t-D_t^s}{D_{t-1}^l(1+\zeta_t^l)(1+\sigma_t^l)})^{1/\delta}$$
"""

def calculate_price(price_PUSD, liquidity_pool, liquidity_pool_next):
  # liquidity_pool = supply - stability_pool
  # liquidity_pool_next = liquidity_pool_previous * drift_liquidity * (1+shock_liquidity)
  price_PUSD_current= price_PUSD * (liquidity_pool / liquidity_pool_next) ** (1/delta)

  return price_PUSD_current

""" **Stabilizers**

There are two stabilizers to attenuate PUSD price deviation from its target range.
...
The amount of $Q_t^d(c)$ is expected to bring the PUSD price back to $1.1+f_t^i$. This means that
...
If $P_t^l < 1-f_t^r$, a fraction $\chi_t$ of PUSD in the liquidity pool is used for redemption
> $$D_t^r = \chi_t D_t^l,$$
...
Note that unlike stabilizer 1, stabilizer 2 has impact of PUSD price in
...
def redeem_trove(accounts, contracts, i, price_ether_current):
    pusd_balance = contracts.pusdToken.balanceOf(accounts[i])
    [firstRedemptionHint, partialRedemptionHintNICR, truncatedPUSDamount] = contracts.hintHelpers.getRedemptionHints(pusd_balance, 70)
    if truncatedPUSDamount == Wei(0):
        return None
    ...
    try:
        ...
        print(f"PUSD Token:    {contracts.pusdToken.address}")
        print(f"i: {i}")
        print(f"account: {accounts[i]}")
        print(f"PUSD bal: {pusd_balance / 1e18}")
        print(f"truncated: {truncatedPUSDamount / 1e18}")
        ...
        print(f"amount: {truncatedPUSDamount}")
...
def price_stabilizer(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_PUSD, index):

    stability_pool = contracts.stabilityPool.getTotalPUSDDeposits() / 1e18
    redemption_pool = 0
    redemption_fee = 0
    ...
    #Calculating Price
    price_PUSD_current = calculate_price(price_PUSD, liquidity_pool, liquidity_pool_next)
    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
    ...
    if price_PUSD_current > 1.1 + rate_issuance:
        supply_wanted = stability_pool + \
                        liquidity_pool_next * \
                        ((1.1+rate_issuance) / price_PUSD)**delta
    ...
    if price_PUSD_current < 1 - rate_redemption:
        ...
        supply_target = stability_pool + \
                        liquidity_pool_next * \
                        ((1-rate_redemption) / price_PUSD)**delta
    ...
    return [price_PUSD_current, redemption_pool, redemption_fee, issuance_PUSD_stabilizer]

"""# PREON Market"""

def PREON_market(index, data):
    #quantity_PREON = (PREON_total_supply/3)*(1-0.5**(index/period))
    ...
    return [price_PREON_current, annualized_earning]
