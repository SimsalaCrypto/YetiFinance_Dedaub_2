...
contract('TroveManager', async accounts => {
  ...
  beforeEach(async () => {
    ...
    const LQTYContracts = await deploymentHelper.deployPREONContracts(bountyAddress, lpRewardsAddress, multisig)
    ...
    await deploymentHelper.connectPREONContractsToCore(LQTYContracts, contracts)
  ...
  it("liquidate(): updates the L_ETH and L_PUSDDebt reward-per-unit-staked totals", async () => {
    ...
    const L_PUSDDebt_expected_1 = C_totalDebt.mul(mv._1e18BN).div(A_collateral.add(B_collateral)).mul(toBN(dec(1, 2)))

    console.log("a", L_PUSDDebt_AfterCarolLiquidated.toString())
    console.log("b", L_PUSDDebt_expected_1.toString())
   ...
   L_PUSDDebt = (180 / 20) + (890 / 10) = 98 PUSD */
    ...
    const L_PUSDDebt_expected_2 = L_PUSDDebt_expected_1.add((B_totalDebt.add(B_increasedTotalDebt).add(B_collateral.mul(L_PUSDDebt_expected_1).div(mv._1e18BN)).mul(mv._1e18BN).div(A_collateral)).mul(toBN(dec(1, 2))))
  ...
  it('getRedemptionHints(): returns 0 as partialRedemptionHintNICR when reaching _maxIterations', async () => {
    ...
    // Get hints for a redemption of 170 + 30 + some extra PUSD. At least 3 iterations are needed
  ...
  it('redeemCollateral(): cancels the provided PUSD with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether', async () => {
    // --- SETUP ---
    ...
    pusdToken.increaseAllowance(troveManagerRedemptions.address, dec(10000, 18), { from: dennis })
    ...
    const PUSDAmount = (th.getEmittedRedemptionValues(redemptionTx)[0]).toString()
    ...
    const expectedTotalETHDrawn = redemptionAmount // convert redemptionAmount PUSD to ETH, at ETH:USD price 200
    console.log('expectedtotaldrawn', expectedTotalETHDrawn.toString())
    const expectedReceivedETH = expectedTotalETHDrawn.sub(toBN(PUSDFee)).div(toBN(200))
    ...
    console.log('pusd after', dennis_PUSDBalance_After.toString())
    console.log('pusd before', dennis_PUSDBalance_Before.toString())
    assert.equal(dennis_PUSDBalance_After, (dennis_PUSDBalance_Before.sub(redemptionAmount)).toString())
  })
  ...
  it('redeemCollateral(): with invalid first hint, zero address', async () => {
    ...
    const dennis_PUSDBalance_After = (await pusdToken.balanceOf(dennis)).toString()
    assert.equal(dennis_PUSDBalance_After, dennis_PUSDBalance_Before.sub(redemptionAmount))
  })
  ...
  it('redeemCollateral(): ends the redemption sequence when the token redemption request has been filled', async () => {
    ...
    const { netDebt: A_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraPUSDAmount: dec(20, 18), extraParams: { from: alice } })
    const { netDebt: B_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraPUSDAmount: dec(20, 18), extraParams: { from: bob } })
    const { netDebt: C_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraPUSDAmount: dec(20, 18), extraParams: { from: carol } })
    const redemptionAmount = A_debt.add(B_debt).add(C_debt)
    const { totalDebt: D_totalDebt, collateral: D_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraPUSDAmount: dec(10, 18), extraParams: { from: dennis } })
    const { totalDebt: E_totalDebt, collateral: E_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraPUSDAmount: dec(10, 18), extraParams: { from: erin } })

    // --- TEST --- 

    // open trove from redeemer.  Redeemer has highest ICR (100ETH, 100 PUSD), 20000%
    const { pusdAmount: F_pusdAmount } = await openTrove({ ICR: toBN(dec(200, 18)), extraPUSDAmount: redemptionAmount.mul(toBN(2)), extraParams: { from: flyn } })

    // skip bootstrapping phase
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_WEEK * 2, web3.currentProvider)

    // Flyn redeems collateral
    pusdToken.increaseAllowance(troveManagerRedemptions.address, dec(100000, 18), { from: flyn })
    await troveManager.redeemCollateral(redemptionAmount, alice, alice, alice, 0, 0, th._100pct, { from: flyn })

    // Check Flyn's redemption has reduced his balance from 100 to (100-60) = 40 PUSD
    const flynBalance = await pusdToken.balanceOf(flyn)

    const PUSDFee = (await troveManager.getRedemptionRate()).mul(redemptionAmount).div(toBN(dec(1,18)))

    // console.log("redemption amount " + redemptionAmount.toString())
    // console.log("FLYN BALANCE : " + flynBalance.toString())
    // console.log("FLYN PUSD AMOUNT BEFORE " + F_pusdAmount.toString())
    // console.log("F PUSD AMOUNT MINUS REDEMPTION AMOUNT: " + (F_pusdAmount.sub(redemptionAmount).toString()))
    // console.log("PUSD FEE CALCULATION: " + (baseRate.mul(redemptionAmount).div(toBN(dec(1,18))).toString()))
    th.assertIsApproximatelyEqual(flynBalance, F_pusdAmount.sub(redemptionAmount).sub(PUSDFee))
  ...
  it('redeemCollateral(): ends the redemption sequence when max iterations have been reached', async () => {
    ...
    const { netDebt: A_debt } = await openTrove({ ICR: toBN(dec(286, 16)), extraPUSDAmount: dec(20, 18), extraParams: { from: alice } })
    const { netDebt: B_debt } = await openTrove({ ICR: toBN(dec(286, 16)), extraPUSDAmount: dec(20, 18), extraParams: { from: bob } })
    const { netDebt: C_debt, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(286, 16)), extraPUSDAmount: dec(20, 18), extraParams: { from: carol } })
    ...
    const PUSDFeeAttempted = (await troveManager.getRedemptionRate()).mul(attemptedRedemptionAmount).div(toBN(dec(1,18)))
    const PUSDFee = (await troveManager.getRedemptionRate()).mul(redemptionAmount).div(toBN(dec(1,18)))
    console.log("PUSD FEE ", PUSDFee.toString())
    console.log("PUSD FEE ATTEMPTED ", PUSDFeeAttempted.toString())
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-40) = 60 PUSD
    const flynBalance = (await pusdToken.balanceOf(flyn)).toString()
    ...
    const PUSDFee2 = (await troveManager.getRedemptionRate()).mul(redemptionAmount).div(toBN(dec(1,18)))
    console.log("PUSD FEE ", PUSDFee2.toString())
  ...
  it("redeemCollateral(): performs partial redemption if resultant debt is > minimum net debt", async () => {
    ...
    await borrowerOperations.openTrove(th._100pct, await getOpenTrovePUSDAmount(dec(10000, 18)), A, A, [contracts.weth.address], [dec(1000, 18)], { from: A })
    // await borrowerOperations.openTrove(th._100pct, await getOpenTrovePUSDAmount(dec(10000, 18)), A, A, { from: A, value: dec(1000, 'ether') })
    ...
    // await borrowerOperations.openTrove(th._100pct, await getOpenTrovePUSDAmount(dec(20000, 18)), B, B, { from: B, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTrovePUSDAmount(dec(30000, 18)), C, C, [contracts.weth.address], [dec(1000, 18)], { from: C })
    // await borrowerOperations.openTrove(th._100pct, await getOpenTrovePUSDAmount(dec(30000, 18)), C, C, { from: C, value: dec(1000, 'ether') })

    // A and C send all their tokens to B
    await pusdToken.transfer(B, await pusdToken.balanceOf(A), {from: A})
    await pusdToken.transfer(B, await pusdToken.balanceOf(C), {from: C})
    ...
    // PUSD redemption is 55000 US
    const PUSDRedemption = dec(55000, 18)
    const tx1 = await th.redeemCollateralAndGetTxObject(B, contracts, PUSDRedemption, th._100pct)
  ...
  it("redeemCollateral(): doesn't perform partial redemption if resultant debt would be < minimum net debt", async () => {
    ...
    // await borrowerOperations.openTrove(th._100pct, await getOpenTrovePUSDAmount(dec(6000, 18)), A, A, { from: A, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTrovePUSDAmount(dec(20000, 18)), B, B, [contracts.weth.address], [dec(1000, 18)], { from: B })
    // await borrowerOperations.openTrove(th._100pct, await getOpenTrovePUSDAmount(dec(20000, 18)), B, B, { from: B, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTrovePUSDAmount(dec(30000, 18)), C, C, [contracts.weth.address], [dec(1000, 18)], { from: C })
    // await borrowerOperations.openTrove(th._100pct, await getOpenTrovePUSDAmount(dec(30000, 18)), C, C, { from: C, value: dec(1000, 'ether') })
  ...
  it('redeemCollateral(): doesnt perform the final partial redemption in the sequence if the hint is out-of-date', async () => {
    ...
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(344, 16)), extraPUSDAmount: dec(8, 18), extraParams: { from: bob } })
    ...
    const expectedTotalETHDrawn = fullfilledRedemptionAmount.sub(frontRunRedepmtion).div(toBN(200)) // redempted PUSD converted to ETH, at ETH:USD price 200
    const expectedReceivedETH = expectedTotalETHDrawn.sub(ETHFee)

    th.assertIsApproximatelyEqual(expectedReceivedETH, receivedETH)

    const dennis_PUSDBalance_After = (await pusdToken.balanceOf(dennis)).toString()
    th.assertIsApproximatelyEqual(dennis_PUSDBalance_After, dennis_PUSDBalance_Before.sub(fullfilledRedemptionAmount.sub(frontRunRedepmtion)))
  })
  ...
  it("redeemCollateral(): doesn't touch Troves with ICR < 110%", async () => {
    ...
    const { pusdAmount: B_pusdAmount, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(133, 16)), extraPUSDAmount: A_debt, extraParams: { from: bob } })

    await pusdToken.transfer(carol, B_pusdAmount, { from: bob })
  ...
  it("redeemCollateral(): finds the last Trove with ICR == 110% even if there is more than one", async () => {
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraPUSDAmount: dec(10, 18), extraParams: { from: whale } })
  ...
  it("redeemCollateral(): reverts if fee exceeds max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraPUSDAmount: dec(80, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraPUSDAmount: dec(90, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraPUSDAmount: dec(100, 18), extraParams: { from: C } })
    ...
    // Max fee is 1%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedPUSDRedemption, dec(1, 16)), "Fee exceeded provided maximum")
  ...
  it("redeemCollateral(): caller can redeem their entire PUSDToken balance", async () => {
    ...
    // Check activePool debt reduced by  400 PUSD
  ...
  it("redeemCollateral(): a redemption made when base rate is zero increases the base rate", async () => {
    ...
    assert.equal(await pusdToken.balanceOf(A), A_balanceBefore.sub(toBN(dec(10, 18))).toString())
  ...
  it("redeemCollateral(): a redemption made when base rate is non-zero increases the base rate, for negligible time passed", async () => {
    ...
    assert.equal(await pusdToken.balanceOf(B), B_balanceBefore.sub(toBN(dec(10, 18))).toString())
  ...
  it("redeemCollateral(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation [ @skip-on-coverage ]", async () => {
    ...
    // Check A's balance has decreased by 10 PUSD
    assert.equal(A_balanceBefore.sub(await pusdToken.balanceOf(A)), dec(10, 18))
  ...
  it('redeemCollateral(): reverts if fee eats up all returned collateral', async () => {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const {
        firstRedemptionHint,
        partialRedemptionHintNICR
      } = await hintHelpers.getRedemptionHints(pusdAmount, 0)
      ...
      await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: bob } })
      await borrowerOperations.adjustTrove(th._100pct, 0, pusdAmount, true, alice, alice, { from: alice, value: pusdAmount.mul(mv._1e18BN).div(price) })
    }
  ...
  it("getPendingPUSDDebtReward(): Returns 0 if there is no pending PUSDDebt reward", async () => {
    ...
    const carolSnapshot_L_PUSDDebt = (await troveManager.rewardSnapshots(carol))[1]
  ...
  it("getTroveStatus(): Returns status", async () => {
    ...
    await openTrove({ ICR: toBN(dec(150, 16)), extraPUSDAmount: B_totalDebt, extraParams: { from: A } })
