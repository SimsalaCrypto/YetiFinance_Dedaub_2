...
contract('BorrowerWrappers', async accounts => {
  ...
  it('claimCollateralAndOpenTrove(): reverts if nothing to claim', async () => {
    ...
    // alice opens Trove
    const { pusdAmount, collateral } = await openTrove({ ICR: toBN(dec(15, 17)), extraParams: { from: alice } })
    ...
    await assertRevert(
      borrowerWrappers.claimCollateralAndOpenTrove(th._100pct, pusdAmount, alice, alice, { from: alice }),
    ...
    th.assertIsApproximatelyEqual(await pusdToken.balanceOf(proxyAddress), pusdAmount)
  ...
  it('claimCollateralAndOpenTrove(): without sending any value', async () => {
    // alice opens Trove
    const { pusdAmount, netDebt: redeemAmount, collateral } = await openTrove({extraPUSDAmount: 0, ICR: toBN(dec(3, 18)), extraParams: { from: alice } })
    // Whale opens Trove
    await openTrove({ extraPUSDAmount: redeemAmount, ICR: toBN(dec(5, 18)), extraParams: { from: whale } })
    ...
    // alice claims collateral and re-opens the trove
    await borrowerWrappers.claimCollateralAndOpenTrove(th._100pct, pusdAmount, alice, alice, { from: alice })

    assert.equal(await web3.eth.getBalance(proxyAddress), '0')
    th.assertIsApproximatelyEqual(await collSurplusPool.getCollateral(proxyAddress), '0')
    th.assertIsApproximatelyEqual(await pusdToken.balanceOf(proxyAddress), pusdAmount.mul(toBN(2)))
    assert.equal(await troveManager.getTroveStatus(proxyAddress), 1)
  ...
  it('claimCollateralAndOpenTrove(): sending value in the transaction', async () => {
    // alice opens Trove
    const { pusdAmount, netDebt: redeemAmount, collateral } = await openTrove({ extraParams: { from: alice } })
    // Whale opens Trove
    await openTrove({ extraPUSDAmount: redeemAmount, ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
    ...
    // alice claims collateral and re-opens the trove
    await borrowerWrappers.claimCollateralAndOpenTrove(th._100pct, pusdAmount, alice, alice, { from: alice, value: collateral })
  ...
  it('claimSPRewardsAndRecycle(): only owner can call it', async () => {
    // Whale opens Trove
    await openTrove({ extraPUSDAmount: toBN(dec(1850, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
    // Whale deposits 1850 PUSD in StabilityPool
    await stabilityPool.provideToSP(dec(1850, 18), ZERO_ADDRESS, { from: whale })

    // alice opens trove and provides 150 PUSD to StabilityPool
    await openTrove({ extraPUSDAmount: toBN(dec(150, 18)), extraParams: { from: alice } })
  ...
  it('claimSPRewardsAndRecycle():', async () => {
    ...
    await openTrove({ extraPUSDAmount: whaleDeposit, ICR: toBN(dec(4, 18)), extraParams: { from: whale } })
    // Whale deposits 1850 PUSD in StabilityPool
    await stabilityPool.provideToSP(whaleDeposit, ZERO_ADDRESS, { from: whale })

    // alice opens trove and provides 150 PUSD to StabilityPool
    const aliceDeposit = toBN(dec(150, 18))
    await openTrove({ extraPUSDAmount: aliceDeposit, ICR: toBN(dec(3, 18)), extraParams: { from: alice } })
    await stabilityPool.provideToSP(aliceDeposit, ZERO_ADDRESS, { from: alice })

    // Defaulter Trove opened
    const { pusdAmount, netDebt, collateral } = await openTrove({ ICR: toBN(dec(210, 16)), extraParams: { from: defaulter_1 } })
    ...
    // Alice PUSDLoss is ((150/2500) * liquidatedDebt)
    const totalDeposits = whaleDeposit.add(aliceDeposit)
    const expectedPUSDLoss_A = liquidatedDebt_1.mul(aliceDeposit).div(totalDeposits)

    const expectedCompoundedPUSDDeposit_A = toBN(dec(150, 18)).sub(expectedPUSDLoss_A)
    ...
    assert.isAtMost(th.getDifference(expectedCompoundedPUSDDeposit_A, compoundedPUSDDeposit_A), 1000)
    ...
    const pusdBalanceBefore = await pusdToken.balanceOf(alice)
    ...
    const proportionalPUSD = expectedETHGain_A.mul(price).div(ICRBefore)
    const borrowingRate = await troveManagerOriginal.getBorrowingRateWithDecay()
    const netDebtChange = proportionalPUSD.mul(mv._1e18BN).div(mv._1e18BN.add(borrowingRate))
    ...
    const pusdBalanceAfter = await pusdToken.balanceOf(alice)
    ...
    assert.equal(pusdBalanceAfter.toString(), pusdBalanceBefore.toString())
    ...
    th.assertIsApproximatelyEqual(troveDebtAfter, troveDebtBefore.add(proportionalPUSD))
    ...
    th.assertIsApproximatelyEqual(depositAfter, depositBefore.sub(expectedPUSDLoss_A).add(netDebtChange))
    // check preon balance remains the same
    th.assertIsApproximatelyEqual(preonBalanceAfter, preonBalanceBefore)

    // PREON staking
    th.assertIsApproximatelyEqual(stakeAfter, stakeBefore.add(expectedPREONGain_A))
  ...
  it('claimStakingGainsAndRecycle(): only owner can call it', async () => {
    ...
    // mint some PREON
    await preonTokenOriginal.unprotectedMint(borrowerOperations.getProxyAddressFromUser(whale), dec(1850, 18))
    await preonTokenOriginal.unprotectedMint(borrowerOperations.getProxyAddressFromUser(alice), dec(150, 18))
    ...
    // Defaulter Trove opened
    const { pusdAmount, netDebt, totalDebt, collateral } = await openTrove({ ICR: toBN(dec(210, 16)), extraParams: { from: defaulter_1 } })

    // skip bootstrapping phase
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_WEEK * 2, web3.currentProvider)

    // whale redeems 100 PUSD
  ...
  it('claimStakingGainsAndRecycle(): reverts if user has no trove', async () => {
    ...
    // Defaulter Trove opened
    const { pusdAmount, netDebt, totalDebt, collateral } = await openTrove({ ICR: toBN(dec(210, 16)), extraParams: { from: defaulter_1 } })
    const borrowingFee = netDebt.sub(pusdAmount)

    // Alice PUSD gain is ((150/2000) * borrowingFee)
    const expectedPUSDGain_A = borrowingFee.mul(toBN(dec(150, 18))).div(toBN(dec(2000, 18)))
  ...
  it('claimStakingGainsAndRecycle(): with only ETH gain', async () => {
    ...
    const proportionalPUSD = expectedETHGain_A.mul(price).div(ICRBefore)
    const borrowingRate = await troveManagerOriginal.getBorrowingRateWithDecay()
    const netDebtChange = proportionalPUSD.mul(toBN(dec(1, 18))).div(toBN(dec(1, 18)).add(borrowingRate))
    ...
    // Alice new PUSD gain due to her own Trove adjustment: ((150/2000) * (borrowing fee over netDebtChange))
    const newBorrowingFee = await troveManagerOriginal.getBorrowingFeeWithDecay(netDebtChange)
    const expectedNewPUSDGain_A = newBorrowingFee.mul(toBN(dec(150, 18))).div(toBN(dec(2000, 18)))
    ...
    assert.equal(ethBalanceAfter.toString(), ethBalanceBefore.toString())
    assert.equal(preonBalanceAfter.toString(), preonBalanceBefore.toString())
    // check proxy pusd balance has increased by own adjust trove reward
    th.assertIsApproximatelyEqual(pusdBalanceAfter, pusdBalanceBefore.add(expectedNewPUSDGain_A))
    // check trove has increased debt by the ICR proportional amount to ETH gain
    th.assertIsApproximatelyEqual(troveDebtAfter, troveDebtBefore.add(proportionalPUSD), 10000)
    // check trove has increased collateral by the ETH gain
    th.assertIsApproximatelyEqual(troveCollAfter, troveCollBefore.add(expectedETHGain_A))
    // check that ICR remains constant
    th.assertIsApproximatelyEqual(ICRAfter, ICRBefore)
  ...
  it('claimStakingGainsAndRecycle(): with only PUSD gain', async () => {
    const price = toBN(dec(200, 18))
    ...
    assert.equal(preonBalanceAfter.toString(), preonBalanceBefore.toString())
    // check proxy pusd balance has increased by own adjust trove reward
    th.assertIsApproximatelyEqual(pusdBalanceAfter, pusdBalanceBefore)
    // check trove has increased debt by the ICR proportional amount to ETH gain
    th.assertIsApproximatelyEqual(troveDebtAfter, troveDebtBefore, 10000)
    // check trove has increased collateral by the ETH gain
    th.assertIsApproximatelyEqual(troveCollAfter, troveCollBefore)
    ...
    th.assertIsApproximatelyEqual(ICRAfter, ICRBefore)
    // check that Stability Pool deposit
    th.assertIsApproximatelyEqual(depositAfter, depositBefore.add(expectedPUSDGain_A), 10000)
    // check preon balance remains the same
    th.assertIsApproximatelyEqual(preonBalanceBefore, preonBalanceAfter)
  ...
  it('claimStakingGainsAndRecycle(): with both ETH and PUSD gains', async () => {
    const price = toBN(dec(200, 18))
    ...
    const borrowingRate = await troveManagerOriginal.getBorrowingRateWithDecay()
    const netDebtChange = proportionalPUSD.mul(toBN(dec(1, 18))).div(toBN(dec(1, 18)).add(borrowingRate))
    const expectedTotalPUSD = expectedPUSDGain_A.add(netDebtChange)
    ...
    th.assertIsApproximatelyEqual(ICRAfter, ICRBefore)
    // check that Stability Pool deposit
    th.assertIsApproximatelyEqual(depositAfter, depositBefore.add(expectedTotalPUSD), 10000)
    // check preon balance remains the same
    th.assertIsApproximatelyEqual(preonBalanceBefore, preonBalanceAfter)
