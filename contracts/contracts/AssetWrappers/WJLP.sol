...
interface IMasterChefJoeV2 {
  ...
  function poolInfo(uint256 _pid) external view returns (PoolInfo memory pool);
...
contract WJLP is ERC20_8, IWAsset {
    ...
    returns (bool success)
  {
    if (
      ...
      if (
        _to != stabilityPool && _to != defaultPool && _to != collSurplusPool
      ) {
    ...
    returns (address[] memory, uint256[] memory)
  {
    ...
    uint256 unclaimed = user.unclaimedJOEReward;
    uint256 pending = ((user.amount * accJoePerShare) / 1e12) - user.rewardDebt;
  ...
  function _sendJoeReward(address _rewardOwner, address _to) internal {
    ...
    uint256 joeToSend = userInfo[_rewardOwner].unclaimedJOEReward;
