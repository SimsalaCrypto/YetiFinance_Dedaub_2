...
contract WBQI is ERC20_8, IWAsset {
  ...
  function accumulateRewards(address _user) internal {
    ...
    if (local.amount > 0) {
      uint256 initialAVAXPerShare;
      uint256 initialQIPerShare;
      ...
      uint256 currentAVAXPerShare = ((address(this).balance -
        globalAVAXRewardPending) * SHAREOFFSET) / _totalSupply;
      uint256 currentQIPerShare = ((QI.balanceOf(address(this)) -
        globalQIRewardPending) * SHAREOFFSET) / _totalSupply;

      uint256 AVAXReward = ((currentAVAXPerShare - initialAVAXPerShare) *
        local.amount) / SHAREOFFSET;
      uint256 QIReward = ((currentQIPerShare - initialQIPerShare) *
        local.amount) / SHAREOFFSET;
  ...
  function _sendReward(address _rewardOwner, address _to) internal {
    ...
    uint256 AVAXToSend = userInfo[_rewardOwner].pendingAVAXReward;
    uint256 QIToSend = userInfo[_rewardOwner].pendingQIReward;
