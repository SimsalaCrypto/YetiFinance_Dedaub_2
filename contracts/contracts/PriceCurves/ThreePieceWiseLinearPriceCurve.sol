...
contract ThreePieceWiseLinearPriceCurve is IPriceCurve, Ownable {
  ...
  function setDecayTime(uint256 _decayTime) external override onlyOwner {
    decayTime = _decayTime;
  }

  // Set the dollar cap in VC terms
  function setDollarCap(uint256 _dollarCap) external override onlyOwner {
  ...
  function _getFee(
    ...
    if (cachedDollarCap != 0) {
      require(
        _totalCollateralVCBalance.add(_collateralVCInput) <= cachedDollarCap,
        "Collateral input exceeds cap"
      );
    }

    uint256 feePre = _getFeePoint(
      _totalCollateralVCBalance,
      _totalVCBalancePre
    );
    uint256 feePost = _getFeePoint(
      _totalCollateralVCBalance.add(_collateralVCInput),
      _totalVCBalancePost
    );

    uint256 decayedLastFee = calculateDecayedFee();
