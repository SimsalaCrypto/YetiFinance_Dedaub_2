...
contract StabilityPool is PoolBase, Ownable, CheckContract, IStabilityPool {
  ...
  function _updateRewardSumAndProduct(
    ...
    for (uint256 i; i < assetsLen; ++i) {
      ...
      uint256 currentAssetS = epochToScaleToSum[asset][currentEpochCached][
        currentScaleCached
      ];
  ...
  function _requireFrontEndNotRegistered(address _address) internal view {
    require(!frontEnds[_address].registered, "SP: Frontend already registered");
  }
  ...
  function _requireValidKickbackRate(uint256 _kickbackRate) internal pure {
    require(_kickbackRate <= DECIMAL_PRECISION, "SP:Invalid Kickback rate");
  }
