...
contract veYETI is IveYETI {
  ...
  modifier onlyYetiController() {
    require(
      msg.sender == address(yetiController),
      "veYETI: Caller is not YetiController"
    );
    _;
  }
    ...
    returns (uint256)
  {
    ...
    return
      (currentVeYeti * userInfo.yetiStakes[_rewarder]) / userInfo.totalYeti;
  }
   ...
   */
  function _allocate(address _user, RewarderUpdate[] memory _yetiAdjustments)
    internal
  {
    UserInfo storage userInfo = users[_user];
    ...
    for (uint256 i; i < nAdjustments; i++) {
      ...
      if (isIncrease) {
        ...
        require(
          userInfo.yetiStakes[rewarder] >= amount,
          "veYETI: insufficient Yeti staked on rewarder"
        );
        userInfo.yetiStakes[rewarder] -= amount;
      }
    }
  ...
  function _handleStaking(
    ...
    if (_amount > 0) {
      if (_isIncreaseStake) {
        ...
        require(
          userInfo.totalYeti >= _amount,
          "veYETI: insufficient Yeti for user to unstake"
        );
        userInfo.totalYeti -= _amount;
        totalYeti -= _amount;
    ...
    // sanity check:
    require(
      totalYeti <= _totalYetiSupply,
      "more Yeti staked in this contract than the total supply"
    );
  }
  ...
  {
    uint256 yetiIncrease = 0;
    uint256 yetiDecrease = 0;
    uint256 n = _adjustments.length;
  ...
  function removeUpdatableRewarder(uint256 _index) external {
    require(msg.sender == contractController, "veYETI: invalid caller");
    ...
    internal
  {
    ...
    for (uint256 i = 0; i < rewarders.length; i++) {
      ...
      if (isUpdatableRewarder[rewarder]) {
        ...
        if (userInfo.totalYeti > 0) {
          veYetiAmount =
            (currentVeYeti * userInfo.yetiStakes[rewarder]) /
            userInfo.totalYeti;
        }
