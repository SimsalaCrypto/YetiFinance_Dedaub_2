...
contract HintHelpers is LiquityBase, Ownable, CheckContract {
  ...
  // TODO
  function getRedemptionHints(uint256 _YUSDamount, uint256 _maxIterations)
    external
    view
    returns (
      address firstRedemptionHint,
      uint256 partialRedemptionHintICR,
      uint256 truncatedYUSDamount
    )
  {
    ISortedTroves sortedTrovesCached = sortedTroves;

    uint256 remainingYUSD = _YUSDamount;
    address currentTroveuser = sortedTrovesCached.getLast();

    while (
      currentTroveuser != address(0) &&
      sortedTroves.getOldICR(currentTroveuser) < MCR
    ) {
    ...
    while (
      ...
      uint256 netYUSDDebt = _getNetDebt(
        troveManager.getTroveDebt(currentTroveuser)
      ).add(troveManager.getPendingYUSDDebtReward(currentTroveuser));

      if (netYUSDDebt > remainingYUSD) {
        // Partial redemption
        if (netYUSDDebt > MIN_NET_DEBT) {
          // MIN NET DEBT = 1800
          uint256 maxRedeemableYUSD = LiquityMath._min(
            remainingYUSD,
            netYUSDDebt.sub(MIN_NET_DEBT)
          );

          uint256 newColl = _calculateVCAfterRedemption(
            currentTroveuser,
            maxRedeemableYUSD
          );
          uint256 newDebt = netYUSDDebt.sub(maxRedeemableYUSD);

          uint256 compositeDebt = _getCompositeDebt(newDebt);
          partialRedemptionHintICR = LiquityMath._computeCR(
            newColl,
            compositeDebt
          );
  ...
  function _calculateVCAfterRedemption(address _borrower, uint256 _YUSDAmount)
    internal
    view
    returns (uint256 newCollVC)
  {
    ...
    )
  {
    ...
    diff = LiquityMath._getAbsoluteDifference(
      _CR,
      sortedTroves.getOldICR(hintAddress)
    );
    ...
    while (i < _numTrials) {
      ...
      uint256 currentICR = sortedTroves.getOldICR(currentAddress);

      // check if abs(current - CR) > abs(closest - CR), and update closest if current is closer
      uint256 currentDiff = LiquityMath._getAbsoluteDifference(currentICR, _CR);
