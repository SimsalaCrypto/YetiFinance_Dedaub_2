  ...
  ReentrancyGuard
{
  ...
  function _singleLeverUp(
    ...
    IYetiRouter router = IYetiRouter(
      contractsCache.whitelist.getDefaultRouterAddress(_token)
    );
    // leverage is 5e18 for 5x leverage. Minus 1 for what the user already has in collateral value.
    uint256 _additionalTokenAmount = _amount.mul(_leverage.sub(1e18)).div(1e18);
    _additionalYUSDDebt = contractsCache.whitelist.getValueUSD(
      _token,
    ...
    uint256 balanceBefore = erc20Token.balanceOf(
      address(contractsCache.activePool)
    );
    ...
    require(
      erc20Token.balanceOf(address(contractsCache.activePool)) ==
  ...
  function _openTroveInternal(
    ...
    vars.RICR = LiquityMath._computeCR(
      _getRVC(_colls, _amounts),
      vars.compositeDebt
    );
  ...
  function _adjustTrove(
    ...
    vars.VCin = contractsCache.whitelist.getValuesVC(
      params._collsIn,
      params._amountsIn
    );
    vars.VCout = contractsCache.whitelist.getValuesVC(
      params._collsOut,
      params._amountsOut
    );
    ...
    vars.currVC = contractsCache.whitelist.getValuesVC(
      vars.currAssets,
      vars.currAmounts
    );
    ...
    if (
      !params._isUnlever && !params._isDebtIncrease && params._YUSDChange != 0
    ) {
    ...
    vars.newRICR = LiquityMath._computeCR(
      _getRVC(vars.newAssets, vars.newAmounts),
      vars.newDebt
    );
  ...
  function _singleUnleverUp(
    ...
    uint256 valueOfCollateral = contractsCache.whitelist.getValueUSD(
      _token,
      _amount
    );
    ...
    require(
      contractsCache.yusdToken.balanceOf(msg.sender) ==
        balanceBefore.add(_finalYUSDAmount),
      "BO:YUSDNotSentUnLever"
    );
  }
  ...
  function _unleverColls(
    ...
    for (uint256 i; i < collsLen; ++i) {
      ...
      if (_maxSlippages[i] != 0) {
        ...
        contractsCache.activePool.sendSingleCollateralUnwrap(
          msg.sender,
          msg.sender,
          _colls[i],
          _amounts[i]
   ...
   */
  function _closeTrove(CloseTrove_Params memory params) internal {
    ContractsCache memory contractsCache = ContractsCache(
      troveManager,
      activePool,
      yusdToken,
        ...
        for (uint256 i; i < colls.length; ++i) {
          ...
          if (j < params._collsOut.length && colls[i] == params._collsOut[j]) {
            _singleUnleverUp(
              contractsCache,
              params._collsOut[j],
              params._amountsOut[j],
              params._maxSlippages[j]
            );
    ...
    _requireSufficientYUSDBalance(
      contractsCache.yusdToken,
      msg.sender,
      debt.sub(YUSD_GAS_COMPENSATION)
    );
  ...
  function _getTotalVariableDepositFee(
    ...
    for (uint256 i; i < tokensLen; ++i) {
      ...
      vars.collateralInputVC = _contractsCache.whitelist.getValueVC(
        vars.token,
        _amountsIn[i]
      );

      // total value in VC of this collateral in active pool (before adding input)
      vars.systemCollateralVC = _contractsCache
        .activePool
        .getCollateralVC(vars.token)
        .add(defaultPool.getCollateralVC(vars.token));

      // (collateral VC In) * (Collateral's Fee Given Yeti Protocol Backed by Given Collateral)
      uint256 whitelistFee = _contractsCache.whitelist.getFeeAndUpdate(
        vars.token,
      ...
      vars.collateralYUSDFee = vars.collateralInputVC.mul(whitelistFee).div(
        1e18
      );
  ...
  function _transferCollateralsIntoActivePool(
    ...
    for (uint256 i; i < amountsLen; ++i) {
      ...
      _singleTransferCollateralIntoActivePool(collAddress, amount);
    }
  }
  ...
  function _triggerDepositFee(IYUSDToken _yusdToken, uint256 _YUSDFee)
    internal
  {
    // Send fee to sYETI contract
  ...
  function _updateTroveFromAdjustment(
    ...
    if (_isDebtIncrease) {
      ...
      newDebt = _troveManager.increaseTroveDebt(
        _borrower,
        _debtChange.add(_variableYUSDFee)
      );
    } else {
  ...
  function _getNewPortfolio(
    ...
    newColls memory newPortfolio = _subColls(
      cumulativeIn,
      _tokensOut,
      _amountsOut
    );
  ...
  function _requireTroveisActive(ITroveManager _troveManager, address _borrower)
    internal
    view
  {
  ...
  function _requireICRisAboveMCR(uint256 _newICR) internal pure {
    require(_newICR >= MCR, "BO:ReqICR>MCR");
  }
  ...
  function _requireNewTCRisAboveCCR(uint256 _newTCR) internal pure {
    require(_newTCR >= CCR, "BO:ReqTCR>CCR");
  }

  function _requireAtLeastMinNetDebt(uint256 _netDebt) internal pure {
    require(_netDebt >= MIN_NET_DEBT, "BO:netDebt<2000");
  }
